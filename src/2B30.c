...
s32 Stage_Open(s32 arg0) {
    ...
    DummiedPrintf3("StageOpen In\n");
    stage = &gStageLoadData[arg0];

    if (stage->counter > 0) {
        if (stage->ramAddr == NULL) {
            DummiedPrintf3("StageOpen Err !!  Ram Address NULL but Counter is Plus\n");
            return -3;
        }
        stage->counter++;
        return -3;
