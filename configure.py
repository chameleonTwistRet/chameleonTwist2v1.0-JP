...
ninja_file.variable('LDFLAGS', '-T chameleonTwist2.jp.ld -T undefined_syms_auto.txt -T undefined_syms.txt -Map build/chameleonTwist2JP.jp.map --no-check-sections')
...
ninja_file.rule('make_z64',
    command = '($OBJCOPY -O binary $in $out) && (sha1sum -c chameleonTwist2JP.sha1)',
...
ninja_file.build("build/chameleonTwist2JP.elf", "make_elf ", o_files)
ninja_file.build("build/chameleonTwist2JP.z64", "make_z64 ", "build/chameleonTwist2JP.elf")
ninja_file.build("build/chameleonTwist2JP.ok", "make_expected ", "build/chameleonTwist2JP.z64")
